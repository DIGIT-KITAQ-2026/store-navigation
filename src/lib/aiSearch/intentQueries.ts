...
const INTENT_QUERIES: { pattern: RegExp; terms: string }[] = [
  /**
   * 「喉が渇いた」は意味検索が緑茶を1位にできるが、「喉が乾いた」だと0件になる。
   * かな漢字変換は「かわいた」を「乾いた」に変換することが多く(iPhoneの既定がこれ)、
   * 実際にデモ中のスマホ入力がこちらになった。表記の違いで結果が変わるのは避ける。
   */
  { pattern: /(喉|のど).{0,3}(渇|乾|かわ)/, terms: "緑茶" },
  // 同じ理由で、飲み物を指す言い回しも意味検索では届かない
  // (実測: 「水が飲みたい」は0件、「飲み物」は携帯用ボトルが1位で緑茶は圏外)
  { pattern: /水分/, terms: "緑茶" },
  { pattern: /(飲み物|飲みもの|のみもの|どりんく)/, terms: "緑茶" },
  { pattern: /(飲みたい|のみたい|飲める)/, terms: "緑茶" },
  { pattern: /(温か|あたたか|暖か).{0,4}(飲|のみ)/, terms: "緑茶 インスタントスープ" },
  /**
