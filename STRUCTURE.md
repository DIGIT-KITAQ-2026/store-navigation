...
- `src/lib/storeFlowState.tsx` — 店舗検索→支店検索→商品検索の進行状態(カードの開閉と
  トップ経由の記録)を画面をまたいで保持するProvider。消費者画面の共通レイアウトに置いており、
  ページ再読み込みで作り直される性質を使って「リロードすると店舗検索からやり直し」を実現している
